...
this will: - Build all items, technologies, recipes etc. to LUA - Package the project to `[FMCS-ROOT]/.mod-preview/dev-mod` (All files included!)
...
this will: - Do all what [Building](#building) does - Create a `mod-list.json` in `[FMCS-ROOT]/.mod-preview/dev-mod` - Start Factorio with mod path set to `[FMCS-ROOT]/.mod-preview/dev-mod`
