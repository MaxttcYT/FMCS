...
const TabTitle = ({
  ...
  return (
    <div
      className={`inline-flex items-center cursor-pointer px-3 py-2 rounded-t font-bold relative accentuated-2 ${
        isActive
          ? `z-20 text-dirty-white bg-gray-dark active ${className || ""}`
          : `z-0 text-black bg-gray-light hover:glow-orange2 hover:bg-orange ${className || ""}`
      }`}
      onClick={onClick}
    >
      <span>{title}</span>
